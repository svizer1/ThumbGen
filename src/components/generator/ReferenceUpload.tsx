'use client'; 
 ...
 import { Palette, X, AlertCircle, Loader2, Sparkles, Search } from 'lucide-react'; 
 ...
 export function ReferenceUpload({ file, onChange, onDescriptionGenerated }: ReferenceUploadProps) { 
  ...
  const handleUnsplashSelect = async (imageUrl: string) => {
    try {
      ...
      const file = new File([blob], 'unsplash-reference.jpg', { type: 'image/jpeg' });
      setFile(file);
    } catch (error) {
      ...
      const file = new File([blob], 'unsplash-reference.jpg', { type: blob.type || 'image/jpeg' });
      setFile(file);
    }
  };
   ...
   if (file) { 
     return ( 
       <div className="space-y-2"> 
         <div className="relative group rounded-xl overflow-hidden border border-[var(--accent)] bg-[var(--bg-base)]"> 
           ...
           <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between"> 
             <div> 
               <div className="flex items-center gap-1.5 mb-0.5"> 
                 ...
                 <span className="text-xs font-medium text-[var(--accent)]">Style Reference</span> 
               </div> 
               <p className="text-[10px] text-[var(--text-primary)]/60 truncate max-w-[160px]">{file.name}</p> 
             </div> 
             <button 
               type="button" 
               onClick={() => { 
         ...
         {analyzing && (
           <div className="flex items-center gap-2 text-xs text-[var(--accent)] bg-[var(--accent-glow)] border border-[var(--accent)]/30 rounded-lg px-3 py-2">
         ...
         {description && !analyzing && (
           <div className="bg-[var(--accent-glow)] border border-[var(--accent)]/30 rounded-lg px-3 py-2">
             <div className="flex items-start gap-2">
               <Sparkles className="w-3.5 h-3.5 text-[var(--accent)] mt-0.5 shrink-0" />
               <div className="flex-1 min-w-0">
                 <p className="text-[10px] font-medium text-[var(--accent)] mb-1">
                   AI описание стиля:
                 </p>
          ...
          <div 
            ...
            className={cn( 
              ...
              dragging 
                ...
                : 'border-[var(--border-default)] bg-[var(--bg-base)] hover:border-[var(--accent)] hover:bg-[var(--accent-glow)]' 
            )} 
          > 
            <div 
              className={cn( 
           ...
           <button
             ...
             className="flex items-center gap-2 px-3 py-1.5 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-lg text-xs text-[var(--text-muted)] hover:text-[var(--accent)] hover:border-[var(--accent)] transition-colors"
           >
