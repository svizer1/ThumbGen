# 🚀 Деплой на Render.com - ОБНОВЛЕНО

## Репозиторий для деплоя

Используйте: **https://github.com/svizer1/ThumbGen**

## Шаги деплоя:

### 1. Откройте Render.com
https://render.com → **New +** → **Background Worker**

### 2. Подключите GitHub
- Выберите репозиторий: `svizer1/ThumbGen`
- Или введите URL: `https://github.com/svizer1/ThumbGen`

### 3. Настройки сервиса

```
Name: thumbgen-telegram-bot
Region: Frankfurt (EU Central)
Branch: main
Root Directory: telegram-bot-python
Build Command: pip install -r requirements.txt
Start Command: python bot.py
```

### 4. Environment Variables

Добавьте эти переменные:

```
TELEGRAM_BOT_TOKEN=8666147272:AAFRTan-PUnbtTZP3sOWKPC_9tSg3e1tXKY

CRYPTOBOT_API_TOKEN=563261:AACkAIRGpD2wQPFp41OUzElzC78xN7e56oH

FIREBASE_ADMIN_PROJECT_ID=thumbgen-3319c

FIREBASE_ADMIN_CLIENT_EMAIL=firebase-adminsdk-fbsvc@thumbgen-3319c.iam.gserviceaccount.com

FIREBASE_ADMIN_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
```

### 5. Deploy!

Нажмите **Create Background Worker**

Render задеплоит бота автоматически!

---

## ✨ Что добавлено в бота:

### Админ панель:
- 📊 **Статистика** - количество пользователей, привязанных аккаунтов
- 📢 **Рассылка** - отправка сообщений всем пользователям
- 👥 **Список пользователей** - последние 10 пользователей

### Команды для админа:
- `/admin` - открыть админ панель
- `/start` - в главном меню будет кнопка "⚙️ Админ панель"

### Ваш Telegram ID уже добавлен как админ: `5887561026`

---

## 🎉 После деплоя:

1. Откройте бота: https://t.me/ThumbGenAI_BOT
2. Отправьте `/start`
3. Нажмите "⚙️ Админ панель"
4. Попробуйте рассылку!

Бот готов! 🚀
